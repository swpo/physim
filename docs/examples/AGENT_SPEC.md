...
`device` is 0 or 1. `port` is an integer from 0 to 11. `amp` is a finite
...
For each device, intervals of the same action kind must not overlap. Intervals
of different kinds, or for different devices, may overlap. Adjacent endpoints
are allowed. These constraints apply even when input values are zero.

List actions in nondecreasing start-time order. At equal times, actions execute
in list order, before measurements requested at that time. Changing that order
