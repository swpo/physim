...
def _norm_jobs(jobs, t0, cap):
    ...
    for j in jobs:
        if isinstance(j, dict):
            ...
            t0s.append(float(j["t0"] if j.get("t0") is not None else t0))
            caps.append(float(j["cap"] if j.get("cap") is not None else cap))
            ics.append(j.get("ic"))
        else:
            g, s = j
            t0s.append(float(t0))
    ...
    if not all(np.isfinite(v) and v > 0 for v in t0s + caps):
        raise ValueError("t0 and cap must be finite positive times")
    if not all(abs(v / CREC - round(v / CREC)) < 1e-9 for v in t0s + caps):
        raise ValueError("t0 and cap must be multiples of CREC")
    if min(t0s) <= BURN:
        raise ValueError(f"t0 and cap must extend beyond the {BURN:g}-unit metric burn-in")
    t0_min = min(t0s)
    bad = []
