...
def required_bundle(config: R6ToolsConfig, *, profile="evaluation") -> Bundle:
    if config.bundle is None:
        raise ValueError("Set task.tools.bundle to a verified local bundle directory before running a model")
    bundle = Bundle(config.bundle, profile=profile)
...
def public_roster(config: R6ToolsConfig):
    return Bundle(config.bundle, profile="simulation").roster if config.bundle is not None else E.E.DEFAULT_ROSTER
...
class R6Task(vf.Task[R6Data, R6State, R6TaskConfig]):
    ...
    async def prediction_reward(self, trace) -> float:
        ...
        if not state.submitted:
            ...
            if not attempted:
                raise vf.TaskError(
                    "No executable predictor was delivered; retry with an appropriate Verifiers limit or harness"
                )
            return 0.0
        context = dict(
            trace_id=trace.id,
    ...
    def load(self):
        ...
        yield R6Task(
            R6Data(
                ...
                timeout=vf.TaskTimeout(setup=self.config.task.setup_timeout, agent=1800, finalize=180, scoring=900),
            ),
            self.config.task,
        )
