...
def plot(source, output):
    ...
    for row, (arm, label) in enumerate(zip(selected, labels)):
        ...
        for col, t in enumerate([0, 10, 50]):
            ...
            if name == "xv":
                partner = fields[np.argmin(abs(times - t)), 1][np.ix_(ys, xs)]
    ...
    title = f"{name}: ordinary interventions change later activator fields"
    if name == "xv":
        title += "\nWhite contour: partner activator"
    ...
    if name == "bf":
        ...
        for field in fields:
            ...
            for a in [0, 1]:
                ...
                coords.append((np.array([ys0.mean(), xs0.mean()]) + 0.5) * 0.5)
        ...
        axes[1].legend()
        causal_path = source / "causal_extended/result.json"
        if not causal_path.exists():
            causal_path = source / "causal/result.json"
        causal = json.loads(causal_path.read_text())
        for label, arm in causal["arms"].items():
            axes[2].plot(arm["times"], arm["separation"], label=label.replace("_", " "))
            axes[3].plot(arm["times"], np.rad2deg(arm["angle_change_rad"]), label=label.replace("_", " "))
        axes[2].set(xlabel="Time (tu)", ylabel="Core separation", title="Coupling control: does the pair stay bound?")
        axes[3].set(
            xlabel="Time (tu)",
            ylabel="Relative angle change (degrees)",
            title="Residual motion after cross-drive removal",
        )
        for axis in axes[2:]:
