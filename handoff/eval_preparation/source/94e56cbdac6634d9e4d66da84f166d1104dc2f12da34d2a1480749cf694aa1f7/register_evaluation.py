...
def register(source, registry_root, name):
    ...
    if (source / "pilot_summary.json").is_file():
        ...
        artifacts["evidence/pilot/config.toml"] = (
            ROOT / "configs/physim" / (origin["world"] + "-pilot.toml")
        ).read_bytes()
    ...
    for p in sorted(upstream.glob("*")):
        if p.name in ("genome.json", "final_state.npz", "protocol.json", "rotation.json", "summary.json"):
    ...
    for archive in (ROOT / "handoff/eval_preparation/source", ROOT / "handoff/evaluation_candidates/fresh/source"):
...
if __name__ == "__main__":
    ...
    if args.command == "add":
        print(json.dumps(register(args.source, args.registry, args.name), indent=2))
    else:
