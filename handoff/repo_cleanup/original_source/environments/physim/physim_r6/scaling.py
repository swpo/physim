...
from physim_r6 import taskset as T
...
class SpendConfig(vf.TaskConfig):
    ...
    def input_ceiling(self):
        return max(self.input_usd_per_mtok, self.cache_read_usd_per_mtok or 0,
                   self.cache_write_usd_per_mtok or 0)
    ...
    def next_call_reserve(self):
        ...
        return (self.context_window * self.input_ceiling +
                self.max_response_tokens * self.output_usd_per_mtok) / 1e6
...
def spend_accounting(trace, config: SpendConfig) -> dict:
    ...
    for usage in usages:
        if usage is None:
            ...
            reserved += ((inputs * config.input_ceiling + outputs *
                          config.output_usd_per_mtok) / 1e6
                         if inputs + outputs else config.next_call_reserve)
    return dict(reported_cost_usd=reported, unreported_cost_reserve_usd=reserved,
        accounted_cost_usd=reported+reserved, calls_missing_cost=missing_cost,
        calls_missing_usage=missing_usage, next_call_reserve_usd=config.next_call_reserve,
        limit_usd=config.limit_usd, model_calls=len(trace.calls),
...
class ScalingTask(T.R6Task):
    ...
    async def setup(self, trace, runtime):
        ...
        trace.info['r6']['scaling'] = dict(spend_config=self.config.spend.model_dump(),
