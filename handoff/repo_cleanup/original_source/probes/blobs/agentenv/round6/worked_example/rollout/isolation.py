...
IMAGE = "physim-r6-pilot:20260908"
...
def docker(arguments, *, data=None, timeout=30):
    result = subprocess.run(["docker", *arguments], input=data, stdout=subprocess.PIPE,
...
class Sandbox:
    def __init__(self, observations, *, artifact=None, image=IMAGE):
        ...
        # Docker Desktop cannot bind this machine's protected Documents tree.
        # Stage ONLY public runtime/data under an owned temporary directory.
        self.public_root = Path(tempfile.mkdtemp(prefix="physim-r6-public-", dir="/private/tmp"))
        ...
        args = ["run", "-d", "--name", self.name, "--network", "none", "--read-only",
                "--cap-drop", "ALL", "--security-opt", "no-new-privileges",
                "--pids-limit", "64", "--cpus", "1", "--memory", "1g", "--memory-swap", "1g",
                "--user", "1000:1000", "--tmpfs", "/tmp:rw,nosuid,nodev,size=128m,mode=1777",
                "--tmpfs", "/output:rw,nosuid,nodev,size=64m,mode=1777",
                "--mount", f"type=bind,source={runtime},target=/runtime,readonly",
                "--mount", f"type=bind,source={self.public_observations},target=/observations,readonly"]
        if artifact is None:
    ...
    def invoke(self, request, *, timeout=30, output_limit=OUTPUT_LIMIT):
        ...
        process = subprocess.Popen(["docker", "exec", "-i", self.name, "python", "/runtime/container_worker.py"],
        ...
        return dict(exit_code=status, output=text[-12000:], output_bytes=len(captured),
    ...
    def export_workspace(self, target, *, excludes=()):
        ...
        stream = subprocess.Popen(["docker", "exec", self.name, "tar", "-C", "/workspace",
                                   *[f"--exclude={pattern}" for pattern in excludes], "-cf", "-", "."],
        ...
        try:
                ...
                for member in archive:
                    ...
                    with archive.extractfile(member) as source, dest.open("xb") as output:
                        while part := source.read(65536):
                            output.write(part)
                    files.append(dict(path=dest.relative_to(target).as_posix(), bytes=member.size,
            ...
            stream.stdout.close(); stream.stderr.close()
    ...
    def prediction(self, actions, queries, *, n_samples, seed):
        # Remove any result from a preceding prediction in this container.
        docker(["exec", self.name, "rm", "-f", "/output/prediction.json"])
        result = self.invoke(dict(mode="predict", actions=actions, queries=queries,
