...
if request["mode"] == "python":
    ...
    # Only actions, queries, sampling seed and sample count cross this boundary.
    ...
    result = module.predict(request["actions"], request["queries"],
    ...
    for query, value in zip(request["queries"], result["samples"]):
        ...
        expected = (request["n_samples"], len(query["t"]), 12, slots[query["sensor"]])
