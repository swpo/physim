...
BASE = os.path.dirname(os.path.abspath(__file__))          # blobkit package dir
CDATA = os.path.join(BASE, "data")            # packaged stamp data [blobkit edit E1]
...
def append_result(record, path=None):
    import fcntl
    path = path or "results.json"     # [blobkit edit E2] default: cwd, not tree
    lockp = path + ".lock"
