...
```bash
uv pip install ./packages/blobkit                # CPU (numpy + scipy)
uv pip install './packages/blobkit[gpu]'         # + jax[cuda12]==0.4.38 for the GPU backend
```
...
Historical provenance and verification transcripts remain under
`probes/blobs/blobkit/` in the Physim research repository. That tree is archival;
new library development belongs in this package. Code is Apache-2.0 (see LICENSE).
