"""g4_battery.py — G4 certification for GE-1 (final candidate).
GE-1: rho=2.1 yW=0.7 leak=0.65 margin=7.0 sig=0.05 over=1.5 r0=0.006
      hazard=4.5e-4 DW=0.02 L=96.  T1=45k settle + T2=15k kick = 60k ticks.
usage: g4_battery.py IDX   (0-3: seeds 0-3; 4-7: jitter configs J0-J3 seed0/1)
Jitter: every SEARCHED theory coord multiplied by U(0.9,1.1) (seeded).
"""
...
idx = int(sys.argv[1])
if idx < 4:
    tc, seed, tag = dict(GE1), idx, f"seed{idx}"
else:
    j = idx - 4
...
out = open(WD + f"/results_g4_{idx}.jsonl", "a")
t0 = time.time()
res, _ = evaluate_cert(tc, seed=seed, T1=45000, T2=15000)
res["phase"] = "g4_battery"; res["tag"] = tag
