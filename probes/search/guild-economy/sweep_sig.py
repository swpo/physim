...
out = open(WD + f"/results_scan5_shard{shard}.jsonl", "a")

grid = []
for rho, leak in [(1.8, 0.55), (2.1, 0.65)]:
    for sig in (0.02, 0.035, 0.05):
        for hz in (5e-4, 7e-4):
            grid.append(dict(rho=rho, yW=0.7, leak=leak, margin=7.0,
                             sig_mut=sig, over=1.5, r0=0.006, hazard=hz))
mine = grid[shard::nsh]
...
for i, tc in enumerate(mine):
    ...
    res["phase"] = "scan5_sig_hz"
    out.write(json.dumps(res, default=float) + "\n"); out.flush()
    s = res.get("seps", {})
    tf = res.get("top_fit", {})
    print(f"[{i+1}/{len(mine)}] rho={tc['rho']} lk={tc['leak']} sig={tc['sig_mut']} hz={tc['hazard']} -> "
