...
for kc in [0.0, 1e-3, 2e-3, 3e-3]:
    r = simulate2(dict(base, kappa_c=kc))
    a = l4_analysis(r["t"], r["nz"][:, 0], r["nz"][:, 1])
    print("kc=%.0e: rho=%.3f net=%.2f exc=%.2f slips=%d T_med=%s T_rate=%s locked=%s"
          % (kc, a["rho"], a["net_wind"], a["max_exc"], a["n_slips"],
             round(a["T_slip"],0) if a["T_slip"] else None,
             round(a["T_slip_rate"],0) if a["T_slip_rate"] else None, a["locked"]), flush=True)
