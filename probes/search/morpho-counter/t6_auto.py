...
for eps in [0.008, 0.016, 0.032]:
    ...
    print("eps=%.3f: uniq=%s nflips=%d Cm=[%.2f,%.2f] top=%s r2=%.3f %s (%.0fs)"
          % (eps, np.unique(n).astype(int), len(flips), r["Cm"][m].min(), r["Cm"][m].max(),
             ft["model"], ft["r2"], {k:v for k,v in ft["params"].items()}, el))
    # print flip times
    ft_times = t[flips][:14]
    print("   flips at t:", [int(x) for x in ft_times], "-> n after:", [int(n[i+1]) for i in flips[:14]])
