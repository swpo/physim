"""verify_v03/relock_032.py — 0.3.2: re-lock assay_batch.py (nf_bucket
helper) + data/fleet/pod_worker_batch.py (nf-bucket grouping + T ordering)."""
import hashlib, json, os, sys, time

PKG = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                   "..", "blobkit"))
LOCKS = os.path.join(PKG, "_locks.json")
RELOCKED = ["assay_batch.py", "data/fleet/pod_worker_batch.py"]
...
def main():
    ...
    out = dict(locked_at=time.strftime("%Y-%m-%d %H:%M:%S"),
               version="0.3.2", files=dict(sorted(files.items())))
