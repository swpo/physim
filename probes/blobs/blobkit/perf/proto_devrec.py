"""proto_devrec.py — PROTOTYPE of 0.4 devrec: device-side REC-grid records.

Pipeline (DESIGN_DEVREC.md; every choice E-row-backed):

  REC-only points: NO host field pull at all. The driver's own pull signal
  is the router — run_chunks calls pull_fn(full) with full=False exactly
  when t is NOT a CREC point and NO snapshot is pending (full_pull_needed);
  that is precisely devrec eligibility. pull_fn2 therefore:
    full=True  -> stock host pull (CREC/orgs/memf/snaps path, unchanged)
    full=False -> dispatch the devrec KERNEL on the CURRENT device buffer
                  (pre-step: the driver pulls before dispatching the next
                  chunk, so the buffer is the correct record-point state;
                  the subsequent step donates G["F"] but the kernel's
                  slice is its own buffer) and return per-lane handles.
  The kernel (one jit, cached per (N, na_max, dtype)): mask (f64 thr,
  host-exact), isfinite, scatter-min root-merge CCL (E1c, outer=5/jumps=8
  + device convergence flag), dense-rank segment stats with f64
  accumulators (E2c policy: ints/max exact, sums <=1e-12), plus an
  acts_snap copy that stays ON DEVICE for fallback lanes.

  Application: flags (tiny) are pulled at the record step; per lane
    good     -> blob rows assembled on host (np.angle f64 — verbatim
                blob_list_fast math) and applied through the REC-grid
                stateful tail (verbatim _record rules).
    fallback (unconverged CCL / MAXL overflow / non-finite) -> stock
                _record on that lane's acts_snap slice (zero-filled
                channel slots, exactly the driver's acts-only pull
                convention). Blowup lanes take this path and exit stock.
  With async_apply=True the pull+assemble+apply of good lanes runs on the
  H-A drain thread (proto_asyncapply.AsyncRecorder): the rung loop never
  blocks on stats; barrier only at rung-final points (H-A contract).

Usage (bench-only):
    import proto_devrec as DR
    DR.install()                   # inline applies (sync, still pull-less)
    DR.install(async_apply=True)   # + H-A drain machinery
    ... run_assay_batch(...) ...
    DR.uninstall()
"""
import atexit
...
def install(async_apply=False, procs=None):
    ...
    AA = None
    if async_apply:
        import proto_asyncapply as PA             # machinery only
        AA = PA
    orig = DRV.run_chunks
    _STATE["orig"] = orig
    st = _STATE["stats"]
