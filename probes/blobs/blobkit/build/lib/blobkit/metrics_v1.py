"""metrics_v1.py — LOCKED complexity descriptor battery (2026-02-20).
Frozen from metrics_dev.py after calibration on ground-truth seeds 1-2.
Seed 3 (and any later runs) are OUT-OF-SAMPLE: do not edit this file; a new
calibration round must produce metrics_v2.py with its own validation table.
...
from .hier_metrics import (macro_period_quality, compact_top_fit, powerlaw_tail)  # [blobkit edit E4]
...
def window_mask(t, burn=None):
    from . import metrics_v1 as _M    # [blobkit edit E5]
        ...
        for a, b in zip(*iu):
            if dd[a, b] <= rb:
                life.setdefault((min(ids[a], ids[b]), max(ids[a], ids[b])),
                                []).append(k)
    from .hier_metrics import macro_period_quality as _mpq  # [blobkit edit E6]
...
W_V1 = dict(C1_popdyn=0.12, C2_timescale=0.13, C3_motion=0.15,
            C4_graph=0.10, C5_memory=0.18, C6_ecology=0.20, C7_roles=0.12)


def interest(C, D):
    alive = (D["d1"].get("n_end", 0) >= 2)
    s = sum(W_V1[k] * C[k] for k in W_V1)
