...
def sim_backend(cfg=None):
    ...
    "gpu_batch" maps to the "gpu" namespace for SINGLE-WORLD paths
    (pod_smoke, legacy pod_worker): the batched ladder itself lives in
    pod_worker_batch.py -> blobkit.assay_batch. [fleetbundle F3, 0.3]"""
    cfg = cfg or config()
    name = cfg.get("sim_backend") or "cpu"
    if name == "gpu_batch":
        name = "gpu"
...
def archive_insert(row, P=None):
    ...
    with locked_json(P["archive"], {}) as c:
        ...
        entry = dict(cand=row["cand"], gen=row.get("gen"), op=row.get("op"),
                     ...
                     vtags=row.get("vtags"),
                     # [v3 continuation] carry the C9 annotation into the archive
                     C9=row.get("C9"), C9_factors=row.get("C9_factors"),
                     spatial_class=row.get("spatial_class"),
                     c9_partial=row.get("c9_partial"),
                     interest_v2=row.get("interest_v2"),
