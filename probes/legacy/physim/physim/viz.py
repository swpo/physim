...
Generates docs/archive/worlds.html: for each difficulty preset, god-view and agent-view
...
Usage: .venv/bin/python -m physim.viz [--out docs/archive/worlds.html]
"""
...
def build_track(track: str, films: bool = True, out_dir="docs/archive") -> str:
    from pathlib import Path
    from physim.legacy_docs import legacy_output, archive_html
    global GIF_DIR
    out = legacy_output(Path(out_dir) / TRACK_META[track]["page"])
    GIF_DIR = str(out.parent / "assets")
...
<a href="scoring.html">How scoring works.</a></p></body></html>""")
    ...
    out.write_text(archive_html(html, out))
    return str(out)


def build(out_path: str = "docs/archive/worlds.html", films: bool = True) -> list[str]:
    from physim.legacy_docs import legacy_output, archive_html
    out = legacy_output(out_path)
...
dynamics, static panels, and the per-world parameter cards.</p>
    ...
    for track in TRACK_META:
        written.append(build_track(track, films=films, out_dir=out.parent))
    return written


if __name__ == "__main__":
    import sys
    out = sys.argv[sys.argv.index("--out") + 1] if "--out" in sys.argv else "docs/archive/worlds.html"
