"""physim.results — generate historical results under docs/archive/.

Curated summary of REPORT.md, split by world track:
  docs/archive/results.html            — overview + headline findings + links
  docs/archive/results-bulk.html       — D-track (D0-D4): grids, strata, cost accounting
  docs/archive/results-chemistry.html  — C-track (C0-C4)
  docs/archive/results-life.html       — B-track (ecology) + E-track (evolution)
  docs/archive/scoring.html            — how scoring works (contracts, CRPS, adequacy)

These describe the earlier benchmark, not the current prediction-function task.
Current public documentation is built by scripts/build_docs.py.
...
def build(out_dir="docs/archive") -> list[str]:
    import pathlib
    from physim.legacy_docs import legacy_output, archive_html
    out = []
    docs = legacy_output(pathlib.Path(out_dir) / "results.html").parent
    ...
    for name, title, body in [
        ...
        p = docs / name
        p.write_text(archive_html(page(title, body), p, shared_assets=True))
        out.append(str(p))
    return out
